...
from ns_admin.utils import runAsyncCmd
import os
        ...
        with ui.row():
            ...
            with ui.card():
                ...
                async def handle_upload(e: events.UploadEventArguments):
                    ...
                    res = await runAsyncCmd(
                        ["sudo", "xc3sprog", "-c", "ftdi", file_path]
                    )
                    ui.notify(f"Flashed {res}", type="positive")

                ui.upload(label="FPGA Config Upload", on_upload=handle_upload).props(
                    "flat color=accent"
                )
